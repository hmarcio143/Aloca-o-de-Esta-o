...
if($_GET){
    ...
    $singleUser = [];
    $query = "SELECT id,dia,mes,name FROM maio WHERE name = '$name' ORDER BY dia ";
    $smtp = $conn->prepare($query);
    $smtp->execute();
    $singleUser = $smtp->fetchAll();

    $QtdLocacao = [];
    $qtd = "SELECT COUNT(name) FROM maio WHERE name='$name'";
...
<?php foreach($singleUser as $single):?>
    <tr class="celula">
        ...
        <td><?=$single["name"]?></td>
        <td>
            <form method="get" action="deleteAlocacaoMaio.php">
