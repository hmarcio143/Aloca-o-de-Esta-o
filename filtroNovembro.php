...
if($_GET){
    ...
    $singleUser = [];
    $query = "SELECT id,dia,mes,name FROM novembro WHERE name = '$name' ORDER BY dia ";
    $smtp = $conn->prepare($query);
    $smtp->execute();
    $singleUser = $smtp->fetchAll();

    $QtdLocacao = [];
    $qtd = "SELECT COUNT(name) FROM novembro WHERE name='$name'";
...
<?php foreach($singleUser as $single):?>
    <tr class="celula">
        ...
        <td><?=$single["name"]?></td>
        <td>
            <form method="get" action="deleteAlocacaoNovembro.php">
